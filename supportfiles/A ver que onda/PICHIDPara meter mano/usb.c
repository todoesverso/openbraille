...
code byte deviceDescriptor[] =
{
    ...
    0x6b, 0x1d, // idVendor (low byte), idVendor (high byte)
    0x02, 0x00, // idProduct (low byte), idProduct (high byte)
    0x10, 0x00, // bcdDevice (low byte), bcdDevice (high byte)
...
code ConfigStruct configDescriptor =
{
    ...
    0x02, 0x07, // bNumEndpoints, bInterfaceClass (HID)
    0x01, 0x02, // bInterfaceSubclass, bInterfaceProtocol,
    0x00,       // iInterface
    // Hid descriptor
  /*  0x09, 0x07, // bLength, bDescriptorType (HID)
    ...
    0x01, 0x02, // bEndpointAddress, bmAttributes (Interrupt)
    HISZ, 0x00, // wMaxPacketSize (low), wMaxPacketSize (high)
    0x00,       // bInterval (1 millisecond)
    // HID Endpoint 1 Out
    0x07, 0x05, // bLength, bDescriptorType (Endpoint)
    0x82, 0x02, // bEndpointAddress, bmAttributes (Interrupt)
...
code byte stringDescriptor1[] =
{
    ...
    'H', 0x00, 'I', 0x00, 'J', 0x00, 'O', 0x00,
    'd', 0x00, 'e', 0x00,
};
code byte stringDescriptor2[] =
{
    0x20, STRING_DESCRIPTOR,
    'P', 0x00, 'U', 0x00, 'T', 0x00, 'A', 0x00,
    ' ', 0x00, 'P', 0x00, 'I', 0x00, 'C', 0x00,
    'C', 0x00, 'H', 0x00, 'O', 0x00, 'T', 0x00,
    'O', 0x00, '!', 0x00, '!', 0x00,
};
...
{   
    ...
    if(UIRbits.ACTIVIF && UIEbits.ACTIVIE)
        UnSuspend();
